...
> **Lưu ý:** Chi tiết từng bước có trong các phần bên dưới. Nếu không dùng Docker được, xem **Cách 2: Chạy không Docker**.
...
| GET | `/partitioning/analyze` | Partitioning analysis (Edge-Cut + Vertex-Cut) |
| GET | `/partitioning/vertex-cut` | Vertex-Cut partitioning (k-way greedy) |
| GET | `/partitioning/multi-level` | Multi-level k-way partitioning (METIS-style) |
| GET | `/topology/analysis` | Deep topology analysis |
| GET | `/topology/clusters` | Connected components |
| GET | `/topology/communities` | Community detection (Louvain) |
...
│   ├── fix_migration.py         # Debug/retry migration script
│   ├── requirements.txt
...
# 2. Coordinator sẽ log: "Site B: [OFFLINE] — queuing pairs for retry"
...
| **NetworkX** | Thuật toán đồ thị (BFS, DFS, Connected Components, Partitioning, Louvain) |
